...
from phonological_bootstrapping.tagging.assign import pick_pos
from analysis.tags import get_frequency_and_activation_for_each_pos
...
def categorize(test_items, weights_matrix, cues2ids, outcomes2ids, method='freq', evaluation='count',
               stats=False, k=50, flush=0, threshold=0, uniphones=True, diphones=False, triphones=False,
               syllables=False, stress_marker=False, boundaries=True):
    ...
    for item in test_items:
        ...
        if boundaries:
            word = '+' + word + '+'
        nphones = encode_item(word, uniphones=uniphones, diphones=diphones,
        ...
        if sorted_outcome_activations[0][1] == 0:
            ...
            if not sorted_outcome_activations[-1][1] > threshold:
                sorted_outcome_activations = [(outcome, act)
                                              for outcome, act in sorted_outcome_activations if act > threshold]
