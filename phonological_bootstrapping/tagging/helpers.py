...
def compute_outcomes_activations(nphones, associations_matrix, cues2ids, outcomes2ids, to_filter):
    ...
    :param cues2ids:            a dictionary mapping cues to their respective row indices. Keys are strings, values are
                                integers.
    :param outcomes2ids:        a dictionary mapping outcomes to their respective column indices. Keys are strings,
                                values are integers.
    ...
    ids = dict(zip(outcomes2ids.values(), outcomes2ids.keys()))
    ...
    outcomes_alphas = {}
    for ii in ids:
        if ids[ii] not in to_filter:
            outcomes_alphas[ids[ii]] = alphas[ii]
